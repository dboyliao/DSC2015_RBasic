...
- | - | - | -
crim | 該鎮犯罪率 | rm | 該鎮平均每戶的房間數目
dis | 該鎮離辦公區的距離 | indus | 該鎮商業比率
...
1. 平均數 rbind->高房價的鎮 犯罪率較低 商業區較少 住宅區比例高  
2. 相關性 -> 房價跟房間數正相關,低收入戶多寡負相關   

---

<p style="font-size:36px">我們想更了解 低收入戶多寡跟房價的關係</p>
```{r}
View(Boston)
...
- 用剛剛學會的 `c` 來建立兩個向量, `taipei_tem` 跟 `hengchun_tem` 紀錄兩地氣溫。
...
<p style="font-size:36px">農業社會 男尊女卑</p>

```{r factor-levels ordinal Argri}
#農業社會 男尊女卑
...
<p style="font-size:36px">阿美族 女尊男卑</p>

```{r factor-levels ordinal Amis}
#阿美族 女尊男卑
...
<p style="font-size:36px"> DataFrame </p>
```{r}
...
<p style="font-size:72px">Let's Play Swirl  
